...
void snapx_clipboard_copy_image(const SnapxImage *img)
{
    ...
    /* Build a GdkPixbuf from RGBA data */
    ...
    if (!dpy) { g_object_unref(pb); return; }

    GdkClipboard *cb = gdk_display_get_clipboard(dpy);
    /* Build a GdkTexture from raw RGBA bytes (avoids deprecated pixbuf API) */
    GBytes *gbytes = g_bytes_new_with_free_func(
        img->data,
        (gsize)(img->stride * img->height),
